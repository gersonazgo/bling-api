  ...
  describe "find" do 
    it "returns a valid NotaFiscal when it exists" do
      VCR.use_cassette('valid_nota_fiscal') do
        ...
        expect(nota_fiscal.numero).to eq("000001")
        ...
        expect(nota_fiscal.dataEmissao).to eq("2016-04-04 00:00:00")
        expect(nota_fiscal.valorNota).to eq("64.90")
        expect(nota_fiscal.chaveAcesso).to eq(nil)
        expect(nota_fiscal.codigosRastreamento).to eq([])
      end
    end
  ...
  describe "all" do 
    ...
    it "returns a valid list of NotaFiscal with filters" do
      VCR.use_cassette('nota_fiscal_list_with_filters') do
        result = Bling::Api::NotaFiscal.all(data_emissao_from: Date.new(2016,4,4), data_emissao_to: Date.new(2016,4,4))
